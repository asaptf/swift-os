...
- **Package and ports fixtures:** `.swpkg` packages, writable package-store
  ...
  static AArch64 Lua, zlib, pcre2, nginx, and sqlite on the host, packages the
  pinned ca-certificates and tzdata bundles as data, publishes all seven into
  one signed seed repository, emits a static-hostable repository root that can
  be served to QEMU, and proves install from a DNS-resolved hosted-style
  repository URL.
