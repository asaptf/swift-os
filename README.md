...
- **Package and ports fixtures:** `.swpkg` packages, writable package-store
  ...
  static AArch64 Lua, zlib, bzip2, zstd, xz, libarchive, pcre2, nginx, and sqlite on the host,
  packages the pinned ca-certificates and tzdata bundles as data, publishes all
  eleven into one signed seed repository, emits a static-hostable repository
