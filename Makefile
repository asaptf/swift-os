...
BASE_EXEC_ELFS := \
	...
	$(BUILD)/glibdemo.elf \
	$(BUILD)/mc.elf
# bash.elf / zsh.elf are intentionally NOT base-image hard deps: those ports are
# not yet building on this host. The base image copies them only if present
# (conditional cp below), so an image can ship with mc on the busybox shell.
...
$(BASE_IMG): $(BASEPACK) $(BASE_SEED_FILES) $(BASE_EXEC_ELFS) $(PKGHELLO_PKG) $(PKGREPO_PUB) $(MODEL_BIN) $(MODEL_TOK) $(MODEL15_Q8) $(MODEL_TOK32) $(MODELMANIFEST) $(MODELSIGN) $(SIGNING_SEED) $(SIGNING_PUB) $(IMG_SIGNING_SEED) $(IMG_SIGNING_PUB) $(SSHD_HOST_SEED_FILE) $(SSHD_KEX_SEED_FILE) $(SSHD_AUTHORIZED_KEYS_FILE) $(NET_IPV6_CONFIG_FILE) $(SWOS_SERVICES_FILE) $(SQLITE_BIN) $(NGINX_BIN) $(NGINX_CERT) $(SITE_SIGNING_PUB) $(SITE_TEST_DEPS) $(OS_STAGE_DEPS) Makefile
	...
	# /bin/sh = busybox (its sh applet runs when argv0 basename is "sh"). The
	# passwd login shell is /bin/sh and there is no standalone shell binary, so
	# without this console-login's execve("/bin/sh") fails for root/user/guest.
	...
	[ -f $(BUILD)/bash.elf ] && cp $(BUILD)/bash.elf $(BASE_ROOT)/bin/bash || true
	[ -f $(BUILD)/zsh.elf ] && cp $(BUILD)/zsh.elf $(BASE_ROOT)/bin/zsh || true
