...
#   make host-test Run fast host-only unit/doc suites (no QEMU).
...
#   make hetzner-os-update-test  Hetzner QEMU gate for live swupdate os (opt-in).
...
# Linux CI defaults come from ci/toolchain.mk (included below).

# ---- Toolchain -------------------------------------------------------------
UNAME_S := $(shell uname -s)
-include ci/toolchain.mk
...
ifdef SWOS_IMG_SIGNING_SEED
IMG_SIGNING_SEED := $(SWOS_IMG_SIGNING_SEED)
endif
...
ifdef SWOS_SITE_SIGNING_SEED
SITE_SIGNING_SEED := $(SWOS_SITE_SIGNING_SEED)
endif
...
# (Linux overrides for LLDLINK / AAVMF_CODE live in ci/toolchain.mk.)
...
OS_STAGE_PACK_CMD := mkdir -p $(BASE_ROOT)/usr/share/swupdate-test; cp $(TEST_BASE_IMG) $(BASE_ROOT)/usr/share/swupdate-test/test-base.img; cp $(TEST_OS_BUNDLE) $(BASE_ROOT)/usr/share/swupdate-test/os.swsys; : > $(BASE_ROOT)/usr/share/swupdate-test/run-os-apply-local-at-boot
else
OS_STAGE_DEPS :=
...
USER_MOUNTPROBE_ELF := $(BUILD)/mountprobe.elf
...
.PHONY: ports-mc-stack-test ports-bash-test ports-zsh-test
...
# Frame pointers required: kernel EL0 fault backtrace walks the AAPCS64 FP chain.
$(BUILD)/user_stackovf.o: userland/stackovf.c userland/lib/syscall.h Makefile | $(BUILD)/.dir
	$(CLANG) $(USER_CFLAGS) -fno-omit-frame-pointer userland/stackovf.c -o $@
...
# V3: runtime mount/unmount driver (invokes SYS_mount/SYS_unmount for v3-mount-test).
...
# /bin/llm: the app wrapper + the shared engine + LM2 matmul pool (WMO).
$(BUILD)/user_llm.o: userland/llm.swift userland/lib/llama2.swift userland/lib/llama_matmul_pool.swift userland/lib/swift_user.h Makefile | $(BUILD)/.dir
	$(SWIFTC) $(USER_SWIFT_FLAGS_NEON) -c userland/llm.swift userland/lib/llama2.swift userland/lib/llama_matmul_pool.swift -o $@

# /bin/llmd: the TCP model-serving daemon + the shared engine + bundle
# verification (manifest parse + sha256) + LM2 matmul pool, compiled together (WMO).
$(BUILD)/user_llmd.o: userland/llmd.swift userland/lib/llama2.swift userland/lib/llama_matmul_pool.swift userland/lib/gguf.swift userland/lib/gguf_engine.swift userland/lib/modelbundle.swift kernel/crypto/sha256.swift kernel/crypto/ed25519.swift kernel/crypto/sha512.swift userland/lib/swift_user.h Makefile | $(BUILD)/.dir
	$(SWIFTC) $(USER_SWIFT_FLAGS_NEON) -c userland/llmd.swift userland/lib/llama2.swift userland/lib/llama_matmul_pool.swift userland/lib/gguf.swift userland/lib/gguf_engine.swift userland/lib/modelbundle.swift kernel/crypto/sha256.swift kernel/crypto/ed25519.swift kernel/crypto/sha512.swift -o $@
...
$(USER_STACKOVF_ELF): $(BUILD)/user_crt0.o $(BUILD)/user_libc.o $(BUILD)/user_stackovf.o userland/user.ld Makefile
	$(LDBIN) $(USER_LDFLAGS) $(BUILD)/user_crt0.o $(BUILD)/user_libc.o $(BUILD)/user_stackovf.o -o $@
...
$(USER_MOUNTPROBE_ELF): $(BUILD)/user_crt0.o $(BUILD)/user_swift_user.o $(BUILD)/user_mountprobe.o userland/user.ld Makefile
	$(LDBIN) $(USER_LDFLAGS) $(BUILD)/user_crt0.o $(BUILD)/user_swift_user.o $(BUILD)/user_mountprobe.o -o $@
...
RELEASEMANIFEST := $(BUILD)/releasemanifest
$(RELEASEMANIFEST): tools/releasemanifest.swift kernel/crypto/sha256.swift Makefile VERSION | $(BUILD)/.dir
	$(HOST_SWIFTC) -O tools/releasemanifest.swift kernel/crypto/sha256.swift -o $@
...
ifdef SWOS_SIGNING_SEED
SIGNING_SEED := $(SWOS_SIGNING_SEED)
endif
...
release-manifest: build base-image $(QEMU_DTB) $(RELEASEMANIFEST)
	...
	SWOS_SWIFT_VERSION="$$($(SWIFTC) --version 2>&1 | head -1)" \
	SWOS_CLANG_VERSION="$$($(CLANG) --version 2>&1 | head -1)" \
	SWOS_GIT_COMMIT="$$(git rev-parse HEAD 2>/dev/null || echo unknown)" \
	SWOS_GIT_DIRTY="$$(test -n "$$(git status --porcelain 2>/dev/null)" && echo true || echo false)" \
		$(RELEASEMANIFEST) build/release-manifest.json $(BUILD)/kernel.elf $(BUILD)/base.img $(BUILD)/virt.dtb
...
# NIC/RNG behind PCIe root ports, and an unlabeled SWDATAFS disk on virtio-blk-pci
# as the /data root; writes files via the login shell, reboots, reads them back.
# V4b covers PCI volumes on plain virt; this is the Hetzner-topology gate.
hetzner-data-pci-test: build $(SSHKEY)
...
# then `swupdate os-apply-local` on the baked SWSYS fixture. Opt-in (slow TCG).
# Rebuilds prod base with INCLUDE_OS_STAGE_TEST=1 for the fixture bundle only.
hetzner-os-update-test: build $(SSHKEY) updatestore $(TEST_BASE_IMG) $(TEST_OS_BUNDLE)
	rm -f $(BASE_IMG) $(DISK_IMG) $(HETZNER_UPDATE_STORE_IMG)
	# sshd-supervised only — prod nginx/llmd load makes TCG encrypted-auth flaky.
...
.PHONY: base-image-prod prod-boot-test hetzner-deploy-build hetzner-deploy-preflight hetzner-deploy-pipeline-test hetzner-deploy-health hetzner-deploy-promote hetzner-update-store hetzner-os-update-test
base-image-prod:
...
# stale binary. CI splits this into dependency-aware jobs (see the three group
# targets below): `.github/workflows/ci-ports.yml` (weekly + workflow_dispatch).
...
# CI group: mc-stack — ncurses + GLib + Midnight Commander (MC needs both).
# Used by ci-ports.yml matrix leg `ports · mc-stack`. Local aggregate remains
# `ports-test`. Elf prereqs force content-stamp rebuilds when inputs are stale.
ports-mc-stack-test: build $(QEMU_DTB) \
		$(BUILD)/ncdemo.elf $(BUILD)/glibdemo.elf $(BUILD)/mc.elf
	rm -f $(BASE_IMG)
	$(MAKE) base-image INCLUDE_NCURSES=1 INCLUDE_GLIB=1 INCLUDE_MC=1
	$(RUNTEST) ./tests/ncurses_test.sh
...
# CI group: bash — builds bash.elf (and ncurses via Makefile dep if needed).
# Used by ci-ports.yml matrix leg `ports · bash`. Packing matches bash-test
# (INCLUDE_BASH only; harness does not need a separate ncurses pack).
ports-bash-test: build $(QEMU_DTB) $(BUILD)/bash.elf
	rm -f $(BASE_IMG)
	$(MAKE) base-image INCLUDE_BASH=1
	$(RUNTEST) ./tests/bash_test.sh
	rm -f $(BASE_IMG); $(MAKE) base-image

# CI group: zsh — builds zsh.elf (and ncurses via Makefile dep if needed).
# Used by ci-ports.yml matrix leg `ports · zsh`. Packing matches zsh-test.
ports-zsh-test: build $(QEMU_DTB) $(BUILD)/zsh.elf
	rm -f $(BASE_IMG)
...
# Hetzner production deploy: SWOSBOOT A/B store (both slots = prod base.img, active A).
# Attached as a virtio-blk-pci volume on the live box; see docs/OS_UPDATE_AUDIT.md.
HETZNER_UPDATE_STORE_IMG := $(BUILD)/hetzner-update-store.img
disk: uefi
	./scripts/make-disk.sh $(DISK_IMG)

hetzner-update-store: updatestore base-image
	$(UPDATESTORE) $(HETZNER_UPDATE_STORE_IMG) A $(BASE_IMG) $(BASE_IMG)
...
.PHONY: node-test npm-test node-http-test npm-install-test node-runtime-test node-docker-smoke-test
# Lightweight gate for the Docker-based Node.js port (NPM35b). Does not build the
# image or compile Node; verifies docker wiring, distfile, and driver scripts.
node-docker-smoke-test: ports/lang/nodejs/Port.json scripts/build-node-docker.sh scripts/link-node.sh docker/Dockerfile.nodebuild
	bash tests/node_docker_smoke_test.sh
...
$(BASE_IMG): $(BASEPACK) $(BASE_SEED_FILES) $(BASE_EXEC_ELFS) $(PKGHELLO_PKG) $(PKGREPO_PUB) $(MODEL_BIN) $(MODEL_TOK) $(MODEL15_Q8) $(MODEL_TOK32) $(MODELMANIFEST) $(MODELSIGN) $(SIGNING_SEED) $(SIGNING_PUB) $(IMG_SIGNING_SEED) $(IMG_SIGNING_PUB) $(SSHD_HOST_SEED_FILE) $(SSHD_KEX_SEED_FILE) $(SSHD_AUTHORIZED_KEYS_FILE) $(NET_IPV6_CONFIG_FILE) $(SWOS_SERVICES_FILE) $(SQLITE_BIN) $(NGINX_BIN) $(NGINX_CERT) $(SITE_SIGNING_PUB) $(SITE_TEST_DEPS) $(OS_STAGE_DEPS) $(KINSTALL_TEST_DEPS) Makefile
	...
	cp $(USER_MOUNTPROBE_ELF) $(BASE_ROOT)/bin/mountprobe
