...
private let sysShmRingCreate: UInt = 101   // shmring_create(pages) → channel id — shared-memory SPSC ring (LA3); needs capNet
private let sysShmRingMap: UInt = 102      // shmring_map(id) → base user VA (or negative errno) — map a channel's pages (LA3)
private let sysShmRingClose: UInt = 103    // shmring_close(id) — drop the creator's base reference to a channel (LA3)
