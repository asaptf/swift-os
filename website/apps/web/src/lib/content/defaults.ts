...
/* Bundled content — grounded in the swift-os repo (README, docs/*, Makefile).
 * Used as the fallback when Strapi is unavailable, and as the seed source for
 * the CMS (apps/cms). Claims are kept defensible: no invented version numbers,
 * no "this page is served by swift-os" overclaim. */

export const REPO = 'https://github.com/asaptf/swift-os';
...
export const defaultLiveProof: LiveProof = {
...
export const defaultStatus: StatusPage = {
...
─── make test · host unit + in-QEMU boot assertions ───`,
	capabilities: [
		{ name: 'AArch64 boot', note: 'cortex-a72 · -kernel + UEFI', status: 'Works', variant: 'ok', detail: 'Deterministic boot to a Swift login in under a second.' },
		{ name: 'QEMU virt machine', note: 'qemu-system-aarch64', status: 'Primary', variant: 'accent', detail: 'The reference target; every gate runs here.' },
		{ name: 'UEFI / GPT boot', note: 'BOOTAA64.EFI · make disk-run', status: 'Works', variant: 'ok', detail: 'Signed image, GPT-partitioned, AAVMF firmware.' },
		{ name: 'Swift userland', note: 'shell + native tools at EL0', status: 'Works', variant: 'ok', detail: 'ls, cat, ps, top, id, httpd, ssh — Embedded Swift.' },
		{ name: 'Three-tier storage', note: 'ro base.img · tmpfs /tmp · /data', status: 'Works', variant: 'ok', detail: 'Signed packed base, volatile /tmp, and a persistent on-disk /data (datafs) that survives reboot; crash-safe SQLite via fsync.' },
		{ name: 'Networking', note: 'in-kernel TCP/IP · virtio-net', status: 'Works', variant: 'ok', detail: 'IPv4, DHCP, DNS, TCP/UDP, ICMP/ARP. IPv6 partial.' },
		{ name: 'HTTP server', note: '/bin/httpd · :8080', status: 'Works', variant: 'ok', detail: 'Concurrent poll-driven static server with MIME + listings.' },
		{ name: 'AI inference', note: '/bin/llmd · TinyStories', status: 'Works', variant: 'ok', detail: 'Native Swift transformer over HTTP. CPU proof, not a GPU runtime.' },
		{ name: 'Package system', note: '.swpkg · signed repo · 12 ports', status: 'Works', variant: 'ok', detail: 'pkg install NAME from a signed static repository.' },
		{ name: 'Capability handles', note: 'principal · typed handles', status: 'Hardening', variant: 'warn', detail: 'Capability bits today; C1–C5 typed handles landed, C6 active.' },
		{ name: 'SMP', note: 'multi-core scheduling', status: 'Active', variant: 'warn', detail: 'Boots with -smp 4; S0–S2 done, S3–S5 in progress.' },
		{ name: 'TLS 1.3', note: 'ChaCha20-Poly1305 · client', status: 'Active', variant: 'warn', detail: 'Client path works; cert verification incomplete.' },
		{ name: 'Linux ABI', note: 'syscall compatibility', status: 'Non-goal', variant: 'muted', detail: 'We remove legacy, not emulate it. Tools are recompiled.' },
		{ name: 'Docker / x86-64', note: 'containers · alt arch', status: 'Non-goal', variant: 'muted', detail: 'Isolation is native; AArch64 only, deliberately.' }
	],
	roadmap: [
		{ phase: 'done', label: 'Boot to Swift login (direct + UEFI)' },
		{ phase: 'done', label: 'MMU process isolation (EL0/EL1)' },
		{ phase: 'done', label: 'In-kernel TCP/IP + <code>/bin/httpd</code>' },
		{ phase: 'done', label: 'TinyStories inference (<code>/bin/llmd</code>)' },
		{ phase: 'done', label: 'Signed base image + .swpkg packages' },
		{ phase: 'done', label: 'Persistent <code>/data</code> filesystem (datafs)' },
		{ phase: 'done', label: 'Try-in-browser (qemu-wasm) demo' },
		{ phase: 'active', label: 'Typed capability handles (C5–C6)' },
		{ phase: 'active', label: 'SMP scheduling (S3–S5)' },
		{ phase: 'active', label: 'Restartable driver services' },
		{ phase: 'active', label: 'TLS 1.3 record layer + cert verification' },
		{ phase: 'planned', label: 'Node.js / JVM hosting (Phase 2)' },
		{ phase: 'planned', label: 'Real-hardware AArch64 boards' }
	],
	nonGoals: [
	...
	coverage: [
		{ to: 12, label: 'ports in the seed repo' },
		{ to: 4, suffix: ' CPUs', label: 'SMP boot tested (-smp 4)' },
		{ to: 0, label: 'panics on the boot gate', accent: true },
		{ to: 51, suffix: '+', label: 'syscalls in the ABI' }
	]
};

export const defaultQuickstart: QuickstartPage = {
	eyebrow: 'Quickstart',
			...
			steps: [
				{
					...
					bodyHtml: 'QEMU via Homebrew, plus the pinned Swift toolchain. Exact versions live in <code>docs/NOTES.md</code> (overridable with <code>make SWIFTC=…</code>).',
					commandTitle: 'Terminal — zsh',
					command: '$ brew install qemu llvm lld\n# Swift toolchain: see docs/NOTES.md (e.g. swift-6.3.2-RELEASE)'
				},
				{
					title: 'Clone the repository',
					commandTitle: 'Terminal — zsh',
					command: '$ git clone https://github.com/asaptf/swift-os\n$ cd swift-os'
				},
				{
					title: 'Cross-build newlib + busybox (once)',
					bodyHtml: 'A one-time step that builds the C runtime and the busybox shell the userland links against.',
					commandTitle: 'Terminal — zsh',
					command: '$ make newlib busybox'
				},
				{
					title: 'Build & boot',
					bodyHtml: 'Build the kernel + signed base image, then launch QEMU on the serial console.',
					commandTitle: 'Terminal — zsh',
					command: '$ make build base-image build/virt.dtb\n$ make run',
					expectedTitle: 'qemu · swift-os',
					expected: `[ ok ] swift-os kernel (Embedded Swift, signed)
[ ok ] MMU enabled · EL1 · 256 MiB
[ ok ] virtio-net up · DHCPv4 10.0.2.15/24
[ ok ] httpd listening on :8080
...
root@swift-os:~$ `
				...
				{
					title: 'Say hello',
					bodyHtml: 'Log in as <code>root</code> / <code>swordfish</code>, then inspect your principal, the network, and the read-only base.',
					commandTitle: 'swift-os shell',
					command: `root@swift-os:~$ id
...
root@swift-os:~$ cat /etc/motd`,
					callout: {
						type: 'tip',
						html: '<strong>Zero-setup option</strong>Want to skip the toolchain? Boot swift-os right in your browser at <a href="/try" style="color:var(--accent)">/try</a>.'
					}
				}
			],
			footWarnHtml:
				'<strong>To quit QEMU</strong>Press <span class="kbd">Ctrl</span> <span class="kbd">A</span> then <span class="kbd">X</span>. Anything you wrote to <code>/tmp</code> is gone on the next boot — that\'s intended.'
		},
		{
			key: 'linux',
			label: 'Linux',
			calloutType: 'note',
			noteHtml:
				'<strong>Linux is a secondary path</strong>The same AArch64 image runs under <code>qemu-system-aarch64</code>. Install QEMU + a recent Swift toolchain, then build and boot exactly as on macOS.',
			commandTitle: 'Terminal — bash',
			command: `$ sudo apt install qemu-system-arm
...
$ make build base-image build/virt.dtb && make run`
		},
		{
			...
			label: 'UEFI boot',
			noteHtml:
				'Boot a GPT-partitioned, signed image through a UEFI loader (<code>BOOTAA64.EFI</code>) under QEMU + AAVMF firmware, instead of QEMU\'s direct <code>-kernel</code> load.',
			commandTitle: 'Terminal',
			command: `$ make disk        # build the GPT disk + EFI loader
...
# or: make uefi-run   (quick FAT dir, no GPT)`
		},
		{
			...
			label: 'Networking',
			noteHtml:
				'swift-os drives virtio-net and acquires <code>10.0.2.15</code> over DHCP on QEMU\'s slirp user network. Inspect and exercise it from inside the guest.',
			commandTitle: 'swift-os shell',
			command: `root@swift-os:~$ netinfo
...
root@swift-os:~$ llmd            # TinyStories inference on :8080`
		...
		{
			key: 'vbox',
			label: 'VirtualBox ARM',
			calloutType: 'warn',
			noteHtml:
				'<strong>Best-effort only</strong>VirtualBox ARM is a manual, non-gated path (see <code>docs/VIRTUALBOX.md</code>). QEMU is the primary, tested target — use the raw image with an EFI-enabled ARM VM if you must.'
		}
	]
};
...
export const defaultArticles: Article[] = [
	{
		...
		bodyMarkdown: `Most kernels are written in C, and for good reasons: it is close to the metal, predictable, and universally supported. But "close to the metal" has always come with a tax — manual memory management, undefined behavior, and a class of bugs that mature tooling only partly contains.
...
- One language across the kernel, the userland, and the host build tools.`
	},
	{
		...
		bodyMarkdown: `swift-os does more than print to a console. Let us be precise about what is live and what is ahead.
...
The flagship profile is application and AI hosting. A native HTTP server and a native inference daemon, both in Embedded Swift on a capability-secured base, are the concrete first proof that the profile is more than a slogan.`
	},
	{
		...
		bodyMarkdown: `A boot is a chain of trust and a sequence of handoffs. swift-os keeps both short, and treats boot time as a product feature.
...
export const defaultFaqs: Faq[] = [
	{ question: 'Is this a Linux clone?', answerHtml: 'No. swift-os removes legacy rather than emulating it. There is no Linux ABI and no Unix-compatibility layer — tools are recompiled against our own POSIX-like syscall surface.', category: 'General' },
	{ question: 'Why Swift for an OS?', answerHtml: 'Embedded Swift gives memory safety and modern ergonomics without a garbage collector or heavy runtime — a good fit for a freestanding kernel. The whole kernel is Swift, with no Foundation or stdlib.', category: 'General' },
	{ question: 'Does it really serve HTTP and run AI?', answerHtml: 'Yes — the in-kernel TCP/IP stack, <code>/bin/httpd</code>, and <code>/bin/llmd</code> (a TinyStories transformer over HTTP) are real and tested. <code>llmd</code> is a CPU proof-of-concept, not a general GPU model runtime. <em>This marketing site</em> is built with SvelteKit + Strapi — but you can boot swift-os in your browser and hit its httpd yourself.', category: 'General' },
	{ question: 'What hardware does it run on?', answerHtml: 'AArch64 under QEMU <code>virt</code> is the reference target (direct <code>-kernel</code> and UEFI/GPT). Real AArch64 boards are planned; x86-64 is a deliberate non-goal.', category: 'Running it' },
	{ question: 'How do I run it?', answerHtml: 'Clone the repo, install QEMU + the Swift toolchain, then <code>make newlib busybox</code> once, and <code>make build base-image build/virt.dtb && make run</code>. See the Quickstart, or try it in your browser with zero setup.', category: 'Running it' },
	{ question: 'Does anything persist across reboots?', answerHtml: 'Yes — anything you write under <code>/data</code>. swift-os has three storage tiers: a signed <strong>read-only</strong> base image, a <strong>volatile</strong> RAM tmpfs at <code>/tmp</code> that vanishes on reboot, and a <strong>persistent</strong> on-disk filesystem (datafs) mounted at <code>/data</code> on its own virtio-blk disk. SQLite databases under <code>/data</code> survive reboot and are crash-safe via <code>fsync</code>; <code>/tmp</code> and the read-only base are not for durable state.', category: 'Running it' },
	{ question: 'Is it multi-core yet?', answerHtml: 'Partly. It boots and runs restricted tests with <code>-smp 4</code> (S0–S2 of the SMP arc are done); full per-CPU scheduling, TLB shootdown, and arbitrary secondary EL0 execution (S3–S5) are active work.', category: 'Running it' },
	{ question: 'Can I run Node.js or a JVM?', answerHtml: 'Not yet. The libc surface, threading model, and syscall set are kept so as not to foreclose them, but Node.js / JVM hosting is recorded Phase-2 design, not something you can run today.', category: 'Roadmap' }
];

// SwiftCube — verified against the swiftcube/ subtree (SC0–SC9b are committed
// with host tests). Manifest + command output match the real CLI exactly:
// swiftcube/manifest (flat compose-like), swiftcube/sctl/Command.swift (output),
// swiftcube/cell/CellSupervisor.swift (lifecycle), docs/SWIFTCUBE_DESIGN.md (§2 topology).
export const defaultSwiftcube: SwiftcubePage = {
