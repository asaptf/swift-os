...
─── make test · host unit + in-QEMU boot assertions ───`,
	capabilities: [
		...
		{ name: 'Two-tier filesystem', note: 'ro base.img + tmpfs /tmp', status: 'Works', variant: 'ok', detail: 'Signed packed base; writable tier is volatile.' },
	...
	roadmap: [
		...
		{ phase: 'planned', label: 'Persistent writable volumes' },
...
export const defaultFaqs: Faq[] = [
	...
	{ question: 'Does anything persist across reboots?', answerHtml: 'Not by default. The base image is read-only and <code>/tmp</code> is a tmpfs that vanishes on reboot. Persistent writable volumes are on the roadmap; for now, persistence is deliberate, never accidental.', category: 'Running it' },
