...
| Driver services | C5a supervisor/service smoke plus C5b/C5c opaque device discovery and handle handoff over endpoint IPC; real MMIO/IRQ/DMA driver handoff remains next |
...
- Many drivers and the network stack still live in the kernel. C5a-C5c prove the
  supervisor/service IPC shape, discovery manifest matching, and opaque
  device-handle ownership transfer; real restartable userland driver services
