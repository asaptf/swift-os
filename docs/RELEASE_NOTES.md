...
- Provides maintainer-side ports scaffolding: `ports/catalog.json`, checked
  Lua, zlib, ca-certificates, pcre2, tzdata, nginx, and sqlite recipes,
...
- Publishes Lua, zlib, ca-certificates, pcre2, tzdata, nginx, and sqlite into
  one signed local seed repository and verifies `pkg install lua`,
  `pkg install zlib`, `pkg install ca-certificates`, `pkg install pcre2`,
  `pkg install tzdata`, `pkg install nginx`, `pkg install sqlite`, Lua smoke
  commands, a `minigzip` round trip, the CA bundle marker, a `pcre2grep` regex
  match, the tzdata zoneinfo marker, nginx version/marker smoke, and a SQLite
  in-memory query with `make package-ports-seed-repo-install-test`.
...
- Package payloads are read-only once active. Local target-side package install
  ...
  checked seed repository can install Lua, zlib, ca-certificates, pcre2,
