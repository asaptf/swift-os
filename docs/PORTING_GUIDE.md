...
`pkg install` by package name, Lua runtime checks, `minigzip`, bzip2, zstd, and
xz compression/decompression smokes, CA and zoneinfo marker reads, a
`pcre2grep` regex match, nginx version/marker checks, a SQLite in-memory query,
and static-host publication from `build/ports-static-host-root`. The hosted URL
