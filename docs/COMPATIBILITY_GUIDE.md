...
| Real userland driver handoff | Not supported yet; C5d surfaces discovered virtio-input metadata but does not grant MMIO/IRQ/DMA ownership |
...
static-host ports fixture for Lua, zlib, ca-certificates, pcre2, tzdata, nginx,
and sqlite, plus a hosted-style HTTP repository URL smoke.
...
- Source-built Lua, zlib, pcre2, and nginx packages plus the ca-certificates/tzdata data packages
