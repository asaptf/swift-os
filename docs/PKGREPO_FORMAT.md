...
- `tests/pkg_static_host_repo_install_test.sh` serves
  ...
  checksums, then boots QEMU and installs Lua, zlib, ca-certificates, pcre2, tzdata, and nginx  from `/aarch64/current`.
...
- `tests/pkg_static_host_dns_repo_install_test.sh` serves the same root through
  ...
  `/bin/pkg` installs Lua, zlib, ca-certificates, pcre2, tzdata, and nginx from that  DNS-resolved repository URL.
