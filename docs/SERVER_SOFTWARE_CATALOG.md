...
| First-party SSHD session/exec preflight | Base `/bin/sshd` accepts a normal OpenSSH client on guest TCP/22, reports `swift-os_sshd-session`, loads its Ed25519 host-key seed from `/etc/ssh/ssh_host_ed25519_seed`, lets host OpenSSH pin the derived host key through known_hosts, supports deploy-specific image-time host-key, KEX seed, and authorized-key staging, completes `curve25519-sha256`/`ssh-ed25519`/`chacha20-poly1305@openssh.com` transport setup, rejects stale fixture keys, authenticates `root` with an `ssh-ed25519` key loaded from `/etc/ssh/authorized_keys`, opens session channels, and executes `/bin/id` plus `/bin/echo HC6-OK` | `make sshd-transport-test`, `make sshd-host-key-rotation-test`, `make sshd-kex-seed-test`, `make sshd-authorized-keys-test` |
...
| `threads` | pthread-like threads, thread-local storage, mutexes, timed condition variables, POSIX semaphores, read/write locks, barriers, atomics, blocking wakeups, and scheduler behavior under many sleeping threads; the current C facade is covered by `pthreadprobe`, `threadsyncprobe`, `uvbarrierprobe`, and `uvcondprobe`. |
