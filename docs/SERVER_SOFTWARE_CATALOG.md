...
| `nodejs` | JavaScript server runtime for modern web apps and tooling. | Node.js. | XL | V8, OpenSSL, zlib, ICU, c-ares or resolver. | `base-posix`, `proc-basic`, `net-client`, `net-server`, `tls-base`, `threads` including pthread semaphores/rwlocks, `mmap-vm` including `largemmapprobe`, `mmapreserveprobe`, and `mapfixedprobe`, signal lifecycle plus later handler frames, high-resolution timers, event notification (`eventfd` plus `poll`/`select`). | Static Node is hard but possible in custom builds. V8 JIT needs an explicit JIT versus jitless policy; fixed-address guard-page behavior is now covered by a C/newlib probe. Native addons are deferred without dynamic loading. | `node -e 'require("http").createServer((_,r)=>r.end("ok")).listen(8080)'` and fetch from host. |
