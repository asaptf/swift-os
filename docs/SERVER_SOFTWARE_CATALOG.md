...
| `threads` | pthread-like threads, thread-local storage, thread names, mutexes including error-check and recursive types, timed condition variables, POSIX semaphores, read/write locks, barriers, atomics, blocking wakeups, atfork handlers, and scheduler behavior under many sleeping threads; the current C facade is covered by `pthreadprobe`, `threadsyncprobe`, `uvsemprobe`, `uvrwlockprobe`, `uvmutexprobe`, `uvthreadnameprobe`, `uvbarrierprobe`, `uvcondprobe`, `uvsignalprobe`, and `uvatforkprobe`. |
