...
| `proc-basic` | `spawn`/`exec`, `waitpid`, pipes, `pipe2`, fd inheritance/close-on-exec, `dup2`, basic signals, and exit status propagation. The libuv-shaped `fork`/`execvp` process path is covered by `uvspawnprobe`; `fork` is not assumed elsewhere unless listed explicitly. |
