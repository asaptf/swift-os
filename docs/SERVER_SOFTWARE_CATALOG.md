...
> `package-ports-seed-repo-install-test` boots SwiftOS with a default repository
> URL, runs `pkg update`, installs `lua` and `zlib`, and runs both package smoke
> commands. Public hosted repositories, remove, upgrade, version-constraint
