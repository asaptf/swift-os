...
| `stackovf` | Deliberate runaway recursion that overflows the user stack; used to prove the kernel's EL0 fault report includes a bounded, collapsed frame-pointer backtrace and that the process dies with SIGSEGV without harming the kernel. | Yes, but prefer the make target. | `make el0-fault-backtrace-test` |
| `satstress` | Kernel-pool saturation smoke: each still-bounded pool (per-process fds, pipes, IPC endpoints) saturates with a clean negative errno rather than a panic and returns to a usable baseline after release (no slot leak); process slots are growable so the process case forks past the initial capacity, reaps fully, and checks baseline return instead of asserting fork refusal; a tmpfs vnode create/unlink churn stays balanced. | Yes, but prefer the make target. | `tests/saturation_test.sh` |
...
| `slotscanstress` | Concurrent full-table process scans (`psinfo`/`sysinfo`/`procstat`) against fork/reap churn that grows and reclaims PMM-backed process-slot segments; asserts no panic, worker success, and free-memory return after the final cycle. Evidence for the scan/shrink UAF fix, not a formal proof. | Yes, but prefer the harness. | `tests/slot_scan_stress_test.sh` |
