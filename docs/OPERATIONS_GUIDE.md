...
inference demo, plus the C5a-C5c restartable driver-service/device-discovery
smoke.
...
| Driver-service status | C5a-C5c pseudo driver-service supervisor, discovery, and opaque device-handle smoke exists; real hardware handoff remains roadmap work |
...
| C5a-C5c driver-service/device-discovery smoke (`-smp 4`) | `make c5-device-discovery-test` |
...
- No general service manager; demos are started manually from the shell. C5a-C5c
  proves a pseudo driver-service restart path, discovery metadata, and opaque
  handle transfer, not a general daemon manager.
