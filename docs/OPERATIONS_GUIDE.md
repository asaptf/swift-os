...
inference demo, plus the C5a/C5b restartable driver-service/device-handle
smoke.
...
| Driver-service status | C5a/C5b pseudo driver-service supervisor and opaque device-handle smoke exists; real hardware handoff remains roadmap work |
...
| C5a/C5b driver-service/device-handle smoke (`-smp 4`) | `make c5-device-handle-test` |
...
- No general service manager; demos are started manually from the shell. C5a/C5b
  proves a pseudo driver-service restart path and opaque handle transfer, not a
