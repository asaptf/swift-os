...
- S5b placement batch (2026-06-10): the restricted S2h EL0 gate now has a
  ...
  CPU, places two `coproc` processes on CPU0 and one on the secondary, captures
  dispatch telemetry before reaping, and logs the S5b marker under `-smp 4`.
  This proves batch placement across CPUs without enabling arbitrary secondary
  scheduling, shared-address-space concurrency, migration, or load balancing.
