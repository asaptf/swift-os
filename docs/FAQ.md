...
cross-builds zlib, bzip2, zstd, xz, pcre2, nginx, and sqlite, packages ca-certificates and
tzdata, publishes all ten into one signed local repository, boots SwiftOS with
...
URL fetching and proves target-side install through a DNS-resolved HTTP
repository hostname:
...
a hosted-style hostname URL. This is still a local fixture, not a public
