...
- Package install exists for local `.swpkg` files and signed static HTTP
  ...
  ports seed publishes Lua, zlib, bzip2, zstd, xz, libarchive, ca-certificates, pcre2, tzdata, nginx,
  and sqlite into one signed local repository, installs all eleven by package
  name, and runs Lua, `minigzip`, bzip2, zstd, xz, `bsdtar`, the CA bundle marker, `pcre2grep`,
