...
- **Not** fully implemented. C1-C5f, C5 proper, the full C6a–C6e cell arc, and C7a/C7b
  (intra-member page cap + per-cell handle cap) exist (accounting → creation → namespace
  → lifecycle → one-service-per-cell → hardened page + handle caps). Richer IPC, an
  optional CPU budget, a persistent restart/FDIR cell supervisor (C7c), nested cells, and
  lifting a production service into a cell (C7d) remain planned work.
