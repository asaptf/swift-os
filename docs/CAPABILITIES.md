...
| **C6c** | Per-cell namespace root (DONE, 2026-06-24) | Implemented slice: `CellSlot.root` carries a VFS root node (0 = unconfined). `SYS_cell_create` grows a `root_path` arg; `vfsApplyCellNamespace` sets a spawned member's C3 confinement root + cwd in `VFSProcessState` to the cell root, so it resolves `/` within the subtree and anything outside (including the global root) is refused by the existing `isDescendant` check. Pure C3 reuse — no per-cell mount table. | `make c6-cell-namespace-test`: `/bin/cellnsprobe` roots a cell at /www, and `/bin/cellnschild` resolves a file inside but is refused /etc and `/`; the default cell stays unconfined; the C3 confinement markers stay green. Single-core + `-smp 4`. | Lifecycle/teardown + optional per-cell limits (C6d) remain. |
