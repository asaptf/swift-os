...
let capLogExport: UInt64      = 1 << 6   // log export; not boot-granted by default
```
...
| **C4a** | Minimal handle-passing IPC | Implemented slice: `endpoint_create`, `ipc_send`, `ipc_recv`, plus the synchronous `ipc_call`/`ipc_reply_recv` reply-port verbs (QW1); byte messages, and one moved handle per message. | Processes exchange bytes and moved handles safely; a server hot loop is one `ipc_reply_recv` per request with kernel-correlated replies (`make ipc-call-test`). | VMOs, async rings, badges, and high-throughput data paths remain future work. |
...
| **C6b** | Cell creation + control handle + spawn-into-cell (DONE, 2026-06-24) | Implemented slice: `HandleKind.cell` (an opaque, transferable capability token, no byte stream); a bounded `cells[]` registry (index 0 = globalCell). `SYS_cell_create(108)` (capConsole) mints a fresh CellId + control-handle fd; `SYS_cell_spawn(109)` launches a child into the cell named by a held handle (**authority by handle** — EBADF without it), re-tagging the child so its resources charge to the new domain. Closing the handle does NOT free the cell (teardown is C6d). | `make c6-cell-create-test`: `/bin/cellcreateprobe` refuses a spawn without the handle, creates cell 2, launches `/bin/cellchild` into it, and asserts the child is charged to cell 2 (not globalCell) and reclaimed on reap. Single-core + `-smp 4`. | Cell namespace root (C6c) and limits/teardown (C6d) remain. The cell shares the global VFS root until C6c. |
| **C6c** | Per-cell namespace root (DONE, 2026-06-24) | Implemented slice: `CellSlot.root` carries a VFS root node (0 = unconfined). `SYS_cell_create` grows a `root_path` arg; `vfsApplyCellNamespace` sets a spawned member's C3 `confineNodes` + cwd to the cell root, so it resolves `/` within the subtree and anything outside (including the global root) is refused by the existing `isDescendant` check. Pure C3 reuse — no per-cell mount table. | `make c6-cell-namespace-test`: `/bin/cellnsprobe` roots a cell at /www, and `/bin/cellnschild` resolves a file inside but is refused /etc and `/`; the default cell stays unconfined; the C3 confinement markers stay green. Single-core + `-smp 4`. | Lifecycle/teardown + optional per-cell limits (C6d) remain. |
| **C6d** | Cell lifecycle: resource cap + enumerate + teardown (DONE, 2026-06-24) | Implemented slice: a hard resident-page cap (`cell_create` `page_cap` arg) refused at spawn-into-cell time (ENOMEM past the ceiling); `SYS_cell_pids(110)` enumerates a cell's members by tag; `SYS_cell_destroy(111)` frees the CellId, refused EBUSY while members live. Monotonic per-cell generation + a generation stamp on the `.cell` handle make a handle stale after destroy + slot reuse (no confused deputy). | `make c6-cell-lifecycle-test`: `/bin/cellcapprobe` caps a cell, spawns until the cap refuses, enumerates members, proves destroy is EBUSY while live, then reaps + frees the CellId and reuses it (stale handle rejected). Single-core + `-smp 4`. | C6e end-to-end payoff + richer limits (handle/CPU caps, intra-member page enforcement) remain. No atomic kernel destroy (§5.3) — teardown is the supervisor walking the enumerated job tree. |
| **C6e** | One service per cell, end to end (DONE, 2026-06-24) | The payoff: `/bin/cellsvcprobe` assembles a cell = { a /www namespace root + a restricted handle set + a resident-page cap }, `cell_spawn`s a real request/reply service `/bin/cellhello` inside it (granted only stdout + the two RPC endpoint ends), drives a live `ping`→`pong` round-trip, confirms `cell_stat` charges it, then stops + reaps it and `cell_destroy`s the cell. The service proves its own isolation (a path outside /www denied; the ungranted fd 0 absent). | `make c6-cell-service-test`: the cell-assembly + the service's isolation proofs + the pong round-trip + `processes=1` + clean teardown + `C6e OK`. Single-core + `-smp 4`. | Richer limits (C7a/C7b), nested cells, and lifting a production service into a cell (C7c/C7d) remain. |
...
- **Not** fully implemented. C1-C5f, C5 proper, the full C6a–C6e cell arc, and C7a
  (intra-member resident-page cap enforcement) exist (accounting → creation → namespace
  → lifecycle → one-service-per-cell → hardened page cap). Richer IPC, the remaining
  per-cell limits (handle cap C7b, optional CPU budget), a persistent restart/FDIR cell
  supervisor (C7c), nested cells, and lifting a production service into a cell (C7d)
  remain planned work.
