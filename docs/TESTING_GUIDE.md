...
existing test scripts or Makefile targets, every complete API example has the
same verification treatment, every executable host Swift tool built from
`tools/*.swift` is covered by
...
- Documentation fence, local-link/anchor, API table, Swift bridge,
  ...
  commands, complete API example verification, command reference, host tool
...
| --- | --- |
| Documentation links/anchors, examples, API tables, Swift bridge coverage, map/front-door coverage, example/API verification blocks/commands, command references, host tool references, or port recipe references | `make docs-test` |
