...
- **Network stack — net-a..e DONE (2026-06-06).** Own Swift, sans-IO. Now hosts a real concurrent server.
  ...
  - **net-c2:** wired the engine to sockets — `socket(SOCK_STREAM)`/`listen`(42)/`accept`(43) + stream
  ...
  - **net-e:** a concurrent poll()-driven HTTP server `/bin/httpd` — one `poll()` loop multiplexes the
