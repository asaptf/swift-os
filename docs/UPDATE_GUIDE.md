...
- Kernel-image staging and activation are implemented for the checked UEFI ESP
  slot path; kernel health rollback is still future work.
...
the alternate signed manifest and selected kernel slot before handoff.
...
| Kernel-image A/B ESP slots | `./tests/uefi_kernel_ab_test.sh`, `./tests/uefi_kstage_test.sh`, `./tests/uefi_kactivate_test.sh` | `UEFI_BOOT=disk ./tests/uefi_boot_test.sh`, `make test` |
