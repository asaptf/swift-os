...
- **L3 (2026-06) — structured records foundation.** Extended `LogEntry` with `detail: UInt64` (0=none). Updated ring initialiser, ringStore, `klog` (now accepts optional trailing `detail: UInt64 = 0` so 3-arg calls are unaffected) and `logDumpRecent` dump formatting (appends " detail=NNN" when nonzero). Added three real example uses (post-heap safe): `klog(..., "timer", "tick rate (Hz)", 100)` after timerInit and two `klog(..., "pmm", "free frames", UInt64(count))` in main.swift reclaim demo; plus `klog(..., "sched", "max threads", 4)` in schedulerInit (the "platform or scheduler" slot per task; early platform avoided to preserve pre-heap ring safety). Work done on topic branch `log/l3-structured` per rules, then --no-ff merged. All L0/L1/L2 strings and "log: recent " header preserved; no boot_test.sh changes. `make build` + direct QEMU serial capture confirmed new detail lines (e.g. "detail=100", "detail=4") only in ring-dump section and no regressions on prior log lines. See `docs/LOGGING.md` L3 entry and phased plan.
