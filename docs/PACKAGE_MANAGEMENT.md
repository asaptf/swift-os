...
> zstd, xz, libarchive, pcre2, nginx, and sqlite, packages ca-certificates and
> tzdata, publishes all eleven into one signed local repository, and can boot SwiftOS with
...
commands, and checked recipes for Lua, zlib, bzip2, zstd, xz, libarchive, ca-certificates, pcre2,
...
artifacts for CA certificates and time zones, publish all eleven packages into one
...
- Keep the real Lua, zlib, bzip2, zstd, xz, libarchive, pcre2, nginx, and sqlite binary package paths plus
...
- `make ports-zstd-repo-fixture` builds static AArch64 zstd CLI tools,
  `libzstd.a`, headers, pkgconf metadata, and a marker file.
- `make ports-xz-repo-fixture` builds static AArch64 xz CLI tools,
  `liblzma.a`, headers, pkgconf metadata, and a marker file.
- `make ports-libarchive-repo-fixture` builds static AArch64 `bsdtar`,
  `libarchive.a`, headers, pkgconf metadata, and a marker file.
...
- `make ports-static-host-publish` creates a deployable static web root for the
  ...
  Lua, zlib, bzip2, zstd, xz, libarchive, ca-certificates, pcre2, tzdata, nginx, and sqlite from that layout
  inside QEMU.
