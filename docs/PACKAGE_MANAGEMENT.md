...
> cross-builds Lua, zlib, pcre2, nginx, and sqlite, packages ca-certificates and
> tzdata, publishes all seven into one signed local repository, and can boot SwiftOS with `/etc/pkg/repo-url` so
