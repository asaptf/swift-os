...
if not {"lua", "zlib", "ca-certificates", "pcre2", "tzdata"}.issubset(names):
...
if [[ "$ok" -eq 1 ]]; then
  echo "PASS: /bin/pkg installed Lua, zlib, ca-certificates, pcre2, and tzdata from the static-host published repository"
  exit 0
fi
