...
if not {"lua", "zlib", "ca-certificates", "pcre2"}.issubset(names):
...
if [[ "$ok" -eq 1 ]]; then
  echo "PASS: /bin/pkg installed Lua, zlib, ca-certificates, and pcre2 from the static-host published repository"
  exit 0
fi
