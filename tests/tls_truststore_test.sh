...
# reachable from the guest at the slirp host alias 10.0.2.2. In the guest a
# dedicated login-shell probe (/bin/tls-ts-probe) runs three cases without
# interactive ash (which faults after console-login on aarch64 Linux CI):
#   A) default verify uses the system store -> test leaf REJECTED
#   B) --cafile <test CA> -> handshake COMPLETES
#   C) --insecure -> handshake COMPLETES
#
...
DISK="$ROOT/build/base-tls-ts.img"
...
if [[ ! -x "$OPENSSL" ]]; then
  command -v openssl >/dev/null 2>&1 && OPENSSL="$(command -v openssl)" \
    || { echo "SKIP: openssl" >&2; exit 0; }
fi

W="$(mktemp -d -t swiftos-tlsts.XXXXXX)"
LOG="$(mktemp -t swiftos-tlsts-log.XXXXXX)"
PIDFILE="$(mktemp -t swiftos-tlsts-pid.XXXXXX)"
INFIFO="$(mktemp -u -t swiftos-tlsts-in.XXXXXX)"; mkfifo "$INFIFO"
QP=""; SPID=""
stop_all() {
  if [[ -f "$PIDFILE" ]]; then
    local p; p="$(cat "$PIDFILE" 2>/dev/null || true)"
    [[ -n "$p" ]] && { kill "$p" 2>/dev/null || true; sleep 0.2; kill -9 "$p" 2>/dev/null || true; }
  fi
...
# Dedicated base image: probe is root's login shell; test CA baked in.
( cd "$ROOT" && make BASE_IMG="$DISK" \
    ROOT_LOGIN_SHELL=/bin/tls-ts-probe \
    TLS_TS_CA_PEM="$W/ca.pem" \
    base-image ) >/dev/null 2>&1 || {
  echo "FAIL: cannot build base-tls-ts.img" >&2
  exit 2
}
...
# Drive past the milestone demos and login; the probe then runs automatically.
require_await "M7 tty: type a line then Enter" 90; send_line 'tty-line'
require_await "M7 tty: running; press Ctrl-C" 40; printf '\003' >&3
require_await "swift-os login:" 60; send_line 'root'
require_await "Password:" 40; send_line 'swordfish'
require_await "TLS-TS-PROBE-START" 90
require_await "TLS-TS-PROBE-DONE" 120
...
grep -qF "handshake failed" <<<"$clean" \
  || fail "default verify did NOT reject a leaf outside the system store"
grep -qF "verifying against 1 trust root(s)" <<<"$clean" \
  || fail "--cafile did not load the test root"
[[ "$(grep -cF 'handshake complete' <<<"$clean")" -ge 2 ]] \
  || fail "verified (--cafile) and/or insecure handshake did not complete"
grep -qF "insecure mode" <<<"$clean" \
  || fail "--insecure was not honored"
grep -qF "TLS-TS-PROBE-DONE" <<<"$clean" \
...
echo "--- serial tail ---" >&2; tail -100 <<<"$clean" >&2
echo "--- tlsget region ---" >&2; sed -n '/tlsget:\|/TLS-TS-PROBE/p' <<<"$clean" | head -60 >&2
exit 1
