#!/usr/bin/env bash
set -u
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
LOG=/tmp/npm_cfg_serial.log
PIDFILE=$(mktemp -t npmcfgser-pid.XXXXXX)
INFIFO=$(mktemp -u -t npmcfgser-in.XXXXXX); mkfifo "$INFIFO"
trap 'kill -9 $(cat "$PIDFILE" 2>/dev/null) 2>/dev/null; rm -f "$LOG" "$PIDFILE" "$INFIFO"' EXIT
pkill -x qemu-system-aarch64 2>/dev/null; sleep 0.3
qemu-system-aarch64 -M virt -cpu cortex-a72 -smp 1 -m 2048M -nographic -no-reboot \
  -pidfile "$PIDFILE" -global virtio-mmio.force-legacy=false \
  -device "loader,file=$ROOT/build/virt-2048.dtb,addr=0x4FF00000,force-raw=on" \
  -drive "file=$ROOT/build/base.img,format=raw,if=none,id=swosbase,readonly=on" \
  -device virtio-blk-device,drive=swosbase \
  -netdev user,id=n0 -device virtio-net-device,netdev=n0 \
  -object rng-random,filename=/dev/urandom,id=rng0 -device virtio-rng-device,rng=rng0 \
  -kernel "$ROOT/build/kernel.elf" <"$INFIFO" >"$LOG" 2>&1 &
exec 3<>"$INFIFO"
await() { local m="$1" n=0; while ((n<900)); do grep -qF "$m" "$LOG" && return 0; sleep 0.1; n=$((n+1)); done; return 1; }
send() { printf '%s\n' "$1" >&3; sleep 0.15; }
await "swift-os login:" 120; send 'root'; await "Password:" 30 || true; send 'swordfish'
await "built-in shell (ash)" 60
send 'export UV_THREADPOOL_SIZE=1 NODE_DEBUG=npm'
send '/bin/node /usr/lib/node_modules/npm/bin/npm-cli.js config list 2>&1 | tail -30'
sleep 12
sed 's/\r//' "$LOG" | grep -iE 'npm|crypto|error|fault|config|debug|EL0' | tail -40