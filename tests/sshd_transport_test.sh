...
# connects with a dev Ed25519 key, completes KEX, authenticates as root, opens a
# session channel, runs /bin/echo, receives stdout, and observes exit-status 0.
...
KEY_SRC="$ROOT/fixtures/ssh/sshd_hc4_ed25519"
...
KEY="$(mktemp -t swiftos-sshd-key.XXXXXX)"
...
trap 'stop_qemu; exec 3>&- 2>/dev/null || true; rm -f "$LOG" "$SSHOUT" "$SSHERR" "$KEY" "$PIDFILE" "$INFIFO"' EXIT
...
"$SSH" -F /dev/null -vvv -p "$HOST_PORT" \
  ...
  root@127.0.0.1 /bin/echo HC4-OK >"$SSHOUT" 2>"$SSHERR" </dev/null
ssh_rc=$?
...
grep -qFx "HC4-OK" "$SSHOUT" \
  || { echo "FAIL: remote stdout was not HC4-OK" >&2; ok=0; }
[[ "$ssh_rc" -eq 0 ]] \
  || { echo "FAIL: ssh exited with $ssh_rc, expected 0" >&2; ok=0; }

if [[ "$ok" -eq 1 ]]; then
  echo "PASS: /bin/sshd authenticated an OpenSSH client and executed /bin/echo over a session channel"
  exit 0
fi
