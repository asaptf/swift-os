...
await "nginx-1.30.2_1" 60 || drive_fail "installed nginx package not listed"
...
if [[ "$ok" -eq 1 ]]; then
  echo "PASS: /bin/pkg installed Lua, zlib, ca-certificates, pcre2, tzdata, and nginx from one signed ports seed repo"
  exit 0
fi
