#!/usr/bin/env bash
# pkg_ports_seed_repo_install_test.sh - P7/P12 smoke: install seed ports from one repo.
...
send_line "echo compressed-zlib-input"
await "compressed-zlib-input" 60 || drive_fail "minigzip compression did not return"
send_line "/usr/bin/minigzip -d /tmp/zlib.txt.gz"
send_line "echo decompressed-zlib-input"
...
await "tzdata-2026b_1" 60 || drive_fail "installed tzdata package not listed"
...
if [[ "$ok" -eq 1 ]]; then
  echo "PASS: /bin/pkg installed Lua, zlib, ca-certificates, pcre2, and tzdata from one signed ports seed repo"
  exit 0
fi
