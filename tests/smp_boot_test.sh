...
# the restricted coproc EL0 pair, an S5b placement batch, and S5c repeated
# placement stress under the same gate.
...
if [[ "$found" -eq 1 ]]; then
  ...
  echo "PASS: SMP boot smoke produced expected S1/S2a-S2h/S3a-S3d/S4a-S4e/S5a-S5c markers with -smp $SMP_CPU_COUNT:"
  while IFS= read -r line; do
    [[ -n "$line" ]] && echo "  - $line"
  done <<<"$EXPECTS"
  exit 0
