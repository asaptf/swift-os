...
LOG=/tmp/npm_malloc_gdb_serial.log
GOUT=/tmp/npm_malloc_gdb.out
: >"$LOG"; : >"$GOUT"
PIDFILE=$(mktemp -t npmmg-pid.XXXXXX)
INFIFO=$(mktemp -u -t npmmg-in.XXXXXX); mkfifo "$INFIFO"
...
cat >/tmp/npm_malloc_gdb.cmds <<EOF
set pagination off
target remote :1239
...
commands
 ...
 printf "crash lr="
 print/x \$x30
 printf "last malloc lr="
 print/x \$last_lr
 info symbol \$last_lr
 info symbol \$x30
 bt 12
 echo NPM-MALLOC-STOP\n
end
continue
EOF

"$GDB" -batch -x /tmp/npm_malloc_gdb.cmds >"$GOUT" 2>&1 &
GPID=$!
sleep 3
...
n=0
while ((n<120)); do grep -q 'NPM-MALLOC-STOP' "$GOUT" 2>/dev/null && break; sleep 0.5; n=$((n+1)); done
kill "$GPID" 2>/dev/null
echo "=== GDB ==="; tail -40 "$GOUT"