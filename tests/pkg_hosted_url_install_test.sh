...
send_line "a=zstd; b=hosted; c=url; d=ok; echo \$a-\$b-\$c-\$d > /tmp/zstd.in"
...
send_line "echo xz-hosted-url-ok | /usr/bin/xz -q -c | /usr/bin/xz -q -d -c"
await "xz-hosted-url-ok" 60 || drive_fail "xz round-trip output mismatch"
...
send_line "cd /tmp && echo libarchive-hosted-url-ok > libarchive.txt && /usr/bin/bsdtar -cf libarchive.tar libarchive.txt && /usr/bin/bsdtar -tf libarchive.tar"
...
grep -qF "xz-hosted-url-ok" <<<"$clean" || { echo "FAIL: xz round-trip output missing" >&2; ok=0; }
...
if [[ "$ok" -eq 1 ]]; then
  echo "PASS: /bin/pkg installed Lua, zlib, bzip2, zstd, xz, libarchive, ca-certificates, OpenSSL, pcre2, curl, tzdata, nginx, and sqlite from hosted repository URL $REPO_URL"
  exit 0
fi
