...
if [[ "$ok" -eq 1 ]]; then
    echo "PASS: UEFI firmware booted swift-os to busybox from $UEFI_BOOT with -smp $SMP_CPU_COUNT (M10/S1/S2a/S2b/S2c/S2d/S2e/S2f/S2g/S2h/S3a/S3b/S3c acceptance)"
    exit 0
fi
echo "--- serial log ---" >&2
sed 's/\r//' "$LOG" >&2
