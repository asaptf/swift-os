...
# llm_serve_tinyllama_test.sh — LM4c acceptance: serve the REAL TinyLlama-1.1B
# model end to end off the dedicated model disk.
#
# Boots the inference profile with extra RAM (a 1.1 GB Q8 model + its KV cache),
# the base disk, the signed TinyLlama model disk (build/model-tinyllama.img,
# mounted read-only at /srv/models by LM3b), and a slirp NIC hostfwd'ing host TCP
# to guest 8080. Logs in, starts /bin/llmd, and asserts:
#   - llmd reports "serving from model disk /srv/models" and selects the
#     "tinyllama" bundle (LM4: llmd prefers /srv/models/tinyllama);
#   - a short POST /completion?n=N returns a COHERENT answer (the model loads and
#     runs end to end — for the prompt "The capital of France is" the greedy
#     answer contains "Paris"), and llmd logs a tokens/sec rate.
#
...
LOG="$(mktemp -t swiftos-tl.XXXXXX)"
OC="$(mktemp -t swiftos-tl-oc.XXXXXX)"
PIDFILE="$(mktemp -t swiftos-tl-pid.XXXXXX)"
INFIFO="$(mktemp -u -t swiftos-tl-in.XXXXXX)"; mkfifo "$INFIFO"
QP=""
stop_qemu() {
  if [[ -f "$PIDFILE" ]]; then
...
if (( ok )); then
  ...
  echo "PASS: TinyLlama-1.1B served from /srv/models (-m $INFER_MEM); coherent answer ('$EXPECT'); $rate"
